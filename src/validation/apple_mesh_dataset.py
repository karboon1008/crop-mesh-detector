"""Apple multi-source Dirichlet mesh dataset preparation -- pools
...
the remaining training pool across 3 nodes. Direct mirror of
tomato_mesh_dataset.py -- see that module's docstring for why no
CornDiseaseView-style label remap is needed here either (the merged item
list is built directly in the canonical compact label space).
"""
...
def _win_long_path(path: str) -> str:
    ...
    (260 chars) -- hit in practice by at least one PlantDoc Apple stock-photo
    filename (data/PlantDoc/train/Apple leaf/apple-tree-branch-...-928225.jpg,
    285 chars once combined with a nested repo checkout path), on machines
    where the registry's LongPathsEnabled is off (the default). No-op on
...
def build_apple_label_map(disease_names: list[str] = APPLE_DISEASE_ORDER) -> AppleLabelMap:
    return AppleLabelMap(
        crop_classes=["Apple"],
...
class AppleMergedDataset(Dataset):
    """A flat, canonical-label-space dataset over pooled multi-source
    Apple items. Exposes the same `.base.samples`/`.base.targets`/
    ...
    def __init__(self, items: list[AppleRawItem], label_map: AppleLabelMap, transform: transforms.Compose):
        self.items = items
...
def build_apple_train_eval_datasets(
    items: list[AppleRawItem], label_map: AppleLabelMap, image_size: int
) -> tuple[AppleMergedDataset, AppleMergedDataset]:
    ...
    train_ds = AppleMergedDataset(items, label_map, train_transform)
    eval_ds = AppleMergedDataset(items, label_map, eval_transform)
    return train_ds, eval_ds


def load_plantvillage_apple_items(root, label_map: AppleLabelMap) -> list[AppleRawItem]:
    dataset = load_full_dataset(root, image_size=32)  # image_size unused beyond this scan
    apple_crop_idx = dataset.labels.crop_classes.index("Apple")
    items: list[AppleRawItem] = []
    for idx in range(len(dataset)):
        class_idx = dataset.base.targets[idx]
        crop_idx, disease_idx = dataset.labels.class_to_crop_disease[class_idx]
        if crop_idx != apple_crop_idx:
            continue
...
def _items_from_paths(
    pairs: list[tuple[str, str]], source: str, label_map: AppleLabelMap
) -> list[AppleRawItem]:
    """Skips any (path, canonical_name) pair whose canonical_name isn't in
    label_map (e.g. a disease excluded via apple_mesh.exclude_diseases,
    such as Black_rot) instead of raising KeyError -- mirrors
    load_plantvillage_apple_items' `continue` behavior for the same case,
    so PlantDoc/PlantWild sources stay consistent with a label_map built
    from a subset of APPLE_DISEASE_ORDER.
    """
    items = []
    for path, canonical_name in pairs:
        if canonical_name not in label_map.name_to_disease_idx:
            continue
        items.append(
            AppleRawItem(source=source, path=path, canonical_disease_idx=label_map.name_to_disease_idx[canonical_name])
        )
    return items


def compute_merged_image_hashes(items: list[AppleRawItem], indices: list[int]) -> dict[int, int]:
    hashes: dict[int, int] = {}
    for idx in indices:
...
def enforce_max_group_size(
    ...
    group's dominant class's pooled count) is broken up -- same fix as
    tomato_mesh_dataset.py's enforce_max_group_size (see that module's
    docstring for the node_1 Soybean collapse this guards against).
    ...
    one sub-group. Only members whose hash is unique within the group
...
def _summarize_partition(items: list[AppleRawItem], shard: list[int], label_map: AppleLabelMap) -> dict:
...
@dataclass
class AppleMeshData:
    train_base: AppleMergedDataset
    eval_base: AppleMergedDataset
    label_map: AppleLabelMap
    image_size: int
    probe_idx: list[int]
    test_idx: list[int]
...
def prepare_apple_mesh_data(cfg) -> AppleMeshData:
    from src.data.plantdoc import load_plantdoc_apple_paths
    from src.data.plantwild import load_plantwild_v1_apple_paths, load_plantwild_v2_apple_paths

    image_size = cfg.get("data.image_size", 160)
    pv_root = cfg.get("data.root", "data/PlantVillage")
    plantdoc_root = cfg.get("apple_mesh.plantdoc_root", "data/PlantDoc")
    plantwild_v1_root = cfg.get("apple_mesh.plantwild_v1_root", "data/PlantWild/plantwild/plantwild/images")
    plantwild_v2_root = cfg.get("apple_mesh.plantwild_v2_root", "data/PlantWild/plantwild_v2/plantwild_v2")
    seed = cfg.get("data.seed", 42)
    num_nodes = cfg.get("apple_mesh.num_nodes", 3)
    dirichlet_alpha = cfg.get("apple_mesh.dirichlet_alpha", 0.3)
    test_fraction = cfg.get("apple_mesh.test_fraction", 0.20)
    dedup_threshold = cfg.get("apple_mesh.dedup_threshold", 5)
    dedup_max_group_size = cfg.get("apple_mesh.dedup_max_group_size", 25)
    probe_fraction = cfg.get("data.probe_set_fraction", 0.05)
    exclude_diseases = set(cfg.get("apple_mesh.exclude_diseases", []) or [])

    disease_names = [name for name in APPLE_DISEASE_ORDER if name not in exclude_diseases]
    label_map = build_apple_label_map(disease_names)

    pv_items = load_plantvillage_apple_items(pv_root, label_map)
    pd_items = _items_from_paths(load_plantdoc_apple_paths(plantdoc_root), "plantdoc", label_map)
    pw1_items = _items_from_paths(
        load_plantwild_v1_apple_paths(plantwild_v1_root), "plantwild_v1", label_map
    )
    pw2_items = _items_from_paths(
        load_plantwild_v2_apple_paths(plantwild_v2_root), "plantwild_v2", label_map
    )
    items = pv_items + pd_items + pw1_items + pw2_items
    if not items:
        raise FileNotFoundError(
            "No Apple images found across PlantVillage/PlantDoc/PlantWild -- check "
            "data.root/apple_mesh.plantdoc_root/apple_mesh.plantwild_v1_root/"
            "apple_mesh.plantwild_v2_root."
        )
    ...
    train_base, eval_base = build_apple_train_eval_datasets(items, label_map, image_size)
    return AppleMeshData(
