"""Stage 2 for the Apple Dirichlet-mesh pipeline: knowledge-transfer
rounds on top of the 3 per-node checkpoints stage 1
(run_apple_pipeline.py) produces, evaluated against the single GLOBAL
held-out test set (not a per-node/cross-node union), plus the
budget-aligned local-only control arm. Direct mirror of
run_tomato_knowledge_transfer.py. Aggregation mechanics (run_kt_round) are
reused unchanged from run_knowledge_transfer.py -- this pipeline changes
the data split, not the aggregation algorithm.
...
def _load_apple_node_from_checkpoint(
    checkpoint_path: Path, data: AppleMeshData, node_id: str, batch_size: int
) -> Node:
    model = build_model(
...
def evaluate_apple_round0_baseline(data: AppleMeshData, stage1_dir: Path, node_ids: list[str]) -> dict[str, dict]:
...
def _per_class_gain_table_apple(
...
def run_apple_round_with_io(
    ...
    for node_id in nodes:
        collective_report = export_and_evaluate_apple(
            nodes[node_id],
            data.label_map,
            data.image_size,
            data.eval_base,
        ...
        control_report = export_and_evaluate_apple(
...
def _describe_data_split_strength(data: AppleMeshData) -> str:
...
def build_apple_knowledge_transfer_summary(
    cfg: Config,
    round0_baseline: dict[str, dict],
    ...
    return {
        ...
        "data_split": {
            "strategy": (
                ...
                "deduplicated multi-source Apple pool (PlantVillage, PlantDoc, PlantWild v1+v2), "
        ...
        "local_only_budget": {
            # Actual epochs/round used for this run -- NOT re-derived from
            # cfg.get("training.distill_epochs_per_round") here, since that
            # silently disagrees with the real value whenever
            # --distill-epochs overrides the config default (as it does for
            # this pipeline's documented `--distill-epochs 10` runs). This
            # is an Appendix A.1 disclosure artifact, so it must report
            # what actually ran, not the config's fallback default.
        ...
        "collaboration_gain_per_disease": _per_class_gain_table_apple(
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="outputs/validation/apple_mesh")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Corn/Tomato pipelines -- pass this flag instead of editing that shared default)"
        ),
    )
    args = parser.parse_args()

    cfg = Config.load(args.config)
    rounds = args.rounds if args.rounds is not None else cfg.get("apple_mesh.rounds", 5)
    if not (1 <= rounds <= 20):
        ...
        for required_name in ("checkpoint.pt", "model.onnx", "manifest.json", "classes.json"):
            ...
            if not required_path.exists():
                raise FileNotFoundError(
                    f"{required_path} not found — run 'python -m src.validation.run_apple_pipeline' first."
                )

    top_level_classes_path = stage1_dir / "classes.json"
    if not top_level_classes_path.exists():
        raise FileNotFoundError(f"{top_level_classes_path} not found — run run_apple_pipeline.py first.")
    ...
    if top_level_classes.get("test_idx") != data.test_idx:
        raise ValueError(
            ...
            f"the split prepare_apple_mesh_data(cfg) produces now. Stage 1 and stage 2 must be run "
    ...
    for node_id in node_ids:
        ...
        if stage1_classes.get("train_idx") != data.per_node[node_id]["train_idx"]:
            raise ValueError(
                ...
                f"(run_apple_pipeline.py) does not match the split prepare_apple_mesh_data(cfg) "
    ...
    round0_baseline = evaluate_apple_round0_baseline(data, stage1_dir, node_ids)
    ...
    nodes = {
        node_id: _load_apple_node_from_checkpoint(
    ...
    # Wrap each node's persisted optimizer in a CosineAnnealingLR spanning
    # the *entire* Stage-2 budget (rounds * distill_epochs), instead of
    # Stage 2 running every round at a flat, un-annealed distill_lr right
    # after Stage 1 annealed down to ~0 -- see the docstring on
    # Node.total_epochs for why that mismatch was producing a visible
    # accuracy dip independent of the optimizer-persistence fix.
    for node in {**nodes, **control_nodes}.values():
        node.total_epochs = rounds * distill_epochs
    ...
    for round_idx in range(1, rounds + 1):
        ...
        summary = run_apple_round_with_io(
            round_idx, nodes, control_nodes, probe_loader, data, cfg, tracker, comm_estimator, round_dir,
            distill_epochs=distill_epochs,
        )
        round_summaries.append(summary)

    kt_summary = build_apple_knowledge_transfer_summary(cfg, round0_baseline, round_summaries, data, distill_epochs)
