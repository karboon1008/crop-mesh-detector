"""Stage 2 for the Tomato+Apple combined Dirichlet-mesh pipeline:
knowledge-transfer rounds on top of the per-node checkpoints stage 1
(run_tomato_apple_pipeline.py) produces, evaluated against the single
GLOBAL held-out test set (not a per-node/cross-node union), plus the
budget-aligned local-only control arm. Direct mirror of
run_apple_knowledge_transfer.py. Aggregation mechanics (run_kt_round) are
reused unchanged from run_knowledge_transfer.py -- this pipeline changes
the data split (now spanning 2 crops), not the aggregation algorithm.

    python -m src.validation.run_tomato_apple_knowledge_transfer --rounds 8 --distill-epochs 2
"""
...
from src.validation.run_tomato_apple_pipeline import node_output_dir
from src.validation.run_knowledge_transfer import _build_per_node_energy_breakdown, run_kt_round
from src.validation.tomato_apple_mesh_dataset import (
    TomatoAppleLabelMap,
    TomatoAppleMeshData,
    prepare_tomato_apple_mesh_data,
)

MODEL_NAME = "mobilenet_v3_small"


def export_and_evaluate_tomato_apple(
    node: Node,
    ...
    """Same shape as run_apple_knowledge_transfer.export_and_evaluate_apple:
    report.json = {"test": ...} against the ONE shared global test set.
    """
...
def _load_tomato_apple_node_from_checkpoint(
    checkpoint_path: Path, data: TomatoAppleMeshData, node_id: str, batch_size: int
) -> Node:
    model = build_model(
...
def evaluate_tomato_apple_round0_baseline(
    data: TomatoAppleMeshData, stage1_dir: Path, node_ids: list[str]
) -> dict[str, dict]:
    """Evaluates each node's ALREADY-EXPORTED stage-1 model.onnx (no
    re-export) against the shared global test set -- round 0's baseline
    for the per-class collaboration-gain table.
    """
...
def _per_class_gain_table_tomato_apple(
...
def run_tomato_apple_round_with_io(
    round_idx: int,
    nodes: dict[str, Node],
    control_nodes: dict[str, Node],
    probe_loader: DataLoader,
    data: TomatoAppleMeshData,
    cfg: Config,
    ...
    for node_id in nodes:
        collective_report = export_and_evaluate_tomato_apple(
            nodes[node_id],
            data.label_map,
            data.image_size,
            data.eval_base,
        ...
        control_report = export_and_evaluate_tomato_apple(
...
def _describe_data_split_strength(data: TomatoAppleMeshData) -> str:
    """Same rationale as run_apple_knowledge_transfer.py's
    `_describe_data_split_strength` -- derives the disclosure string from
    the ACTUAL partition_diagnostics rather than asserting full class
    ...
    if min_classes_present >= num_classes:
        return (
            ...
            f"{num_classes} canonical disease classes (spanning Apple + Tomato); skew is "
            "proportional (Dirichlet-drawn over the (crop, disease) pair), not exclusionary"
        )
    ...
    return (
        "skew is proportional (Dirichlet-drawn over the (crop, disease) pair), but NOT every node "
        f"has training exposure to all {num_classes} canonical disease classes: at least one node "
        f"has zero training samples for some classes -- missing classes by node: {missing_by_node}"
    )


def build_tomato_apple_knowledge_transfer_summary(
    cfg: Config,
    round0_baseline: dict[str, dict],
    ...
    return {
        ...
        "data_split": {
            "strategy": (
                f"Dirichlet (crop, disease)-pair skew partition (alpha={dirichlet_alpha}) over a "
                "merged, deduplicated multi-source, multi-crop (Apple + Tomato) pool (PlantVillage, "
                "PlantDoc, PlantWild v1+v2), with a global test split carved out before partitioning"
            ),
            "strength": _describe_data_split_strength(data),
            "sources": ["PlantVillage", "PlantDoc", "PlantWild_v1", "PlantWild_v2"],
            "crops": data.label_map.crop_classes,
            "dirichlet_alpha": dirichlet_alpha,
            "partition_diagnostics": data.partition_diagnostics,
        },
        "local_only_budget": {
            "epochs_per_round": distill_epochs,
            "lr": cfg.get("training.distill_lr", 0.0005),
            "rounds": len(round_summaries),
        },
        "collective_budget": {
            "distill_epochs_per_round": distill_epochs,
            "lr": cfg.get("training.distill_lr", 0.0005),
            "kd_weight": cfg.get("training.kd_weight", 0.5),
            "proto_weight": cfg.get("training.proto_weight", 0.5),
            "rounds": len(round_summaries),
        ...
        "collaboration_gain_per_disease": _per_class_gain_table_tomato_apple(
        ...
        "limitation_note": (
            ...
            "Crop-head gain is expected to be trivial once a node has seen >=1 sample of each crop, "
            "since Apple vs Tomato is a comparatively easy visual distinction next to within-crop "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Local-supervised epochs per round for both arms (default: "
            "training.distill_epochs_per_round in config.yaml, currently shared with "
            "Corn/Tomato/Apple pipelines -- pass this flag instead of editing that shared default)"
        ),
    )
    args = parser.parse_args()

    cfg = Config.load(args.config)
    rounds = args.rounds if args.rounds is not None else cfg.get("tomato_apple_mesh.rounds", 5)
    if not (1 <= rounds <= 20):
        parser.error(f"--rounds must be between 1 and 20, got {rounds}")
    distill_epochs = (
        args.distill_epochs if args.distill_epochs is not None else cfg.get("training.distill_epochs_per_round", 1)
    )
        ...
        for required_name in ("checkpoint.pt", "model.onnx", "manifest.json", "classes.json"):
            ...
            if not required_path.exists():
                raise FileNotFoundError(
                    f"{required_path} not found — run 'python -m src.validation.run_tomato_apple_pipeline' first."
                )

    top_level_classes_path = stage1_dir / "classes.json"
    if not top_level_classes_path.exists():
        raise FileNotFoundError(f"{top_level_classes_path} not found — run run_tomato_apple_pipeline.py first.")
    ...
    if top_level_classes.get("test_idx") != data.test_idx:
        raise ValueError(
            ...
            f"the split prepare_tomato_apple_mesh_data(cfg) produces now. Stage 1 and stage 2 must "
            f"be run against the same config and the same source data — otherwise stage 2 would "
            f"silently evaluate checkpoints on images they were trained on."
        )
    for node_id in node_ids:
        ...
        if stage1_classes.get("train_idx") != data.per_node[node_id]["train_idx"]:
            raise ValueError(
                ...
                f"(run_tomato_apple_pipeline.py) does not match the split "
                f"prepare_tomato_apple_mesh_data(cfg) produces now. Stage 1 and stage 2 must be run "
                f"against the same config and the same source data — otherwise stage 2 would "
                f"silently evaluate this node's checkpoint on images it was trained on."
            )

    print("=== round 0 baseline (stage 1 checkpoints, global test set) ===")
    round0_baseline = evaluate_tomato_apple_round0_baseline(data, stage1_dir, node_ids)
    ...
    nodes = {
        node_id: _load_tomato_apple_node_from_checkpoint(
    ...
    for round_idx in range(1, rounds + 1):
        ...
        summary = run_tomato_apple_round_with_io(
            round_idx, nodes, control_nodes, probe_loader, data, cfg, tracker, comm_estimator, round_dir,
            distill_epochs=distill_epochs,
        )
        round_summaries.append(summary)

    kt_summary = build_tomato_apple_knowledge_transfer_summary(cfg, round0_baseline, round_summaries, data, distill_epochs)
