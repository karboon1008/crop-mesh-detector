"""CLI entrypoint chaining the Apple Dirichlet-mesh validation pipeline
across 3 nodes: train -> export -> evaluate. Direct mirror of
run_tomato_pipeline.py, adapted for Apple's merged multi-source pool and
a single GLOBAL held-out test set (not per-node).

    python -m src.validation.run_apple_pipeline
    python -m src.validation.run_apple_pipeline --stage train
    python -m src.validation.run_apple_pipeline --stage export
    python -m src.validation.run_apple_pipeline --stage evaluate
...
from src.validation.apple_mesh_dataset import AppleMeshData, prepare_apple_mesh_data
...
def run_train_stage(
    data: AppleMeshData, node_id: str, output_dir: Path, epochs: int = 2, pretrained: bool = True
) -> None:
...
def run_export_stage(data: AppleMeshData, node_id: str, output_dir: Path) -> None:
    ...
    directly -- same rationale as run_tomato_pipeline.py's
    run_export_stage: `data` is freshly recomputed via
    prepare_apple_mesh_data(cfg) on every separate CLI invocation, so using
    its freshly-recomputed scalars/indices here could silently disagree
    with the split `train` actually trained against if config or source
    data drifted between invocations.
    """
...
def run_evaluate_stage(data: AppleMeshData, node_id: str, output_dir: Path) -> None:
