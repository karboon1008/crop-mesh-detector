...
def test_prepare_corn_mesh_data_raises_on_disease_name_not_in_dataset(corn_scoped_config):
    """A disease name that doesn't match any name in
    dataset.labels.disease_classes at all (e.g. a typo, or the wrong
    underscore/space separator vs. the real PlantVillage folder naming)
    must fail loudly rather than silently producing zero indices.
    """
    cfg, root = corn_scoped_config
    cfg_dict = cfg.as_dict()
    cfg_dict["corn_mesh"]["node_diseases"]["node_1"] = "Cercospora_leaf_spot_Gray_leaf_spot"
    bad_cfg = Config(cfg_dict)

    with pytest.raises(ValueError, match="node_1"):
        prepare_corn_mesh_data(bad_cfg)


def test_prepare_corn_mesh_data_raises_on_empty_disease_indices(tmp_path):
    """A disease name that IS a valid, real disease_classes entry
    dataset-wide (so the "not present at all" check alone wouldn't catch
    it) but has zero matching samples under Corn specifically (it
    belongs only to another crop) must still fail loudly rather than
    silently leaving that node with only its healthy share.
    """
    import numpy as np
    ...
    class_counts = {
        ...
        "Tomato___Bacterial_spot": 4,  # real disease name, but not under Corn
    }
    ...
    cfg = Config(
        {
            ...
            "corn_mesh": {
                ...
                "node_diseases": {
                    "node_0": "Common_rust",
                    "node_1": "Bacterial_spot",  # valid dataset-wide, zero for Corn
